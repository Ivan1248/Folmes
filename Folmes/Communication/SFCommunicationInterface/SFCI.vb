...
Partial Public Class SharedFolderCI : Implements ICommunication
    Public Event NewCommonMessage(message As Message) Implements ICommunication.NewCommonMessage
    Public Event NewPrivateMessage(message As Message) Implements ICommunication.NewPrivateMessage
    Public Event PongReceived(rtt_in_ms As Long) Implements ICommunication.PongReceived
    ...
    Public Sub Start(SynchronizingObject As Form) Implements ICommunication.Start
        EnableFSWatchers(SynchronizingObject)
        PingPong.CleanPing()
    End Sub

    Public Sub SendMessage(channel As String, message As Message) Implements ICommunication.SendMessage
        ...
        End If
        Dirs.Create(dirPath)
        Dim filePath As String = Path.Combine(dirPath, Convert.ToString(message.Time, 16) & Files.Extension.Message)
        MessageFile.Create(filePath, message)
    End Sub

    Public Function Ping(username As String) As Boolean Implements ICommunication.Ping
        Return PingPong.Ping(username)
    End Function

    Public Sub GetOldMessages(channel As String, count As Integer, loadSubRef As ICommunication.MessageLoadingSub) Implements ICommunication.GetOldMessages
