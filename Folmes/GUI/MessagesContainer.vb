...
Public Class MessagesContainer : Inherits WebBrowser
    Private _msgContainer As HtmlElement

    Private Const DefaultHtml As String = "<html><head></head><body style=""background-color:#222""></body></html>"

    Public Event MsgContainerLoaded()
    ...
    Public Sub LoadBaseHtml(scripts As String())
...
#End Region
    ...
    Public Sub LoadMessageToOutput(message As Message)
        Dim removeOldest As Boolean = False
        If _htmlMessages.Count >= My.Settings.NofMsgs Then
            removeOldest = True
            _htmlMessages.RemoveOldest()
        End If
        Dim messageElement As HtmlElement = InsertMessage(message, removeOldest)
        _htmlMessages.InsertElement(messageElement, message.Time, _msgContainer)
        ScrollDown()
        RefreshScroller()
    End Sub

    Public Function InsertMessage(message As Message, removeOldest As Boolean) As HtmlElement
        If removeOldest Then RemoveOldestHtmlMessage()
